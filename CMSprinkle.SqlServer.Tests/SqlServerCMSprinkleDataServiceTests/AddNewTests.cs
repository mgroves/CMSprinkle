...
public class AddNewTests : SqlServerIntegrationTest
{
    ...
    public async Task Trying_to_add_duplicate_key_throws_exception()
    {
        ...
        Assert.That(ex.AllExceptions().Any(x => x.Message.Contains("Violation of PRIMARY KEY constraint")), Is.True);
        Assert.That(ex.AllExceptions().Any(x => x.Message.Contains("Cannot insert duplicate key in object")), Is.True);
        Assert.That(ex.AllExceptions().Any(x => x.Message.Contains($"The duplicate key value is ({model.Key})")), Is.True);
    }
